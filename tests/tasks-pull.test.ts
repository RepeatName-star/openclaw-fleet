...
test("POST /v1/tasks/pull reclaims expired leased tasks", async () => {
  const db = initTestDb();
  await runMigrations(db);
  const pool = createTestPool(db);
  ...
  const task = await pool.query(
    "insert into tasks (target_type, target_id, action, payload, status, attempts, lease_expires_at) values ($1,$2,$3,$4,'leased',1, now() - interval '5 seconds') returning id",
    ["instance", instanceId, "skills.update", {}],
  );
  ...
  expect(row.rows[0].status).toBe("leased");
});
